...
前面的模拟结果，锐利的波形经过若干时间的迭代后就会变得模糊，这是典型的数值耗散。

核心原因在于我们使用差分替代了偏导，在这一步引入了误差。

为了解决误差，可以使用其他线性公式拟合偏导。将偏导数泰勒展开，选取前面几项。

这样做会增加计算量，但可以避免一定程度的耗散。

选择不同的离散化方法也是一个trade-off过程，离散化就是使用线性系统替代模型中的非线性系统，不可避免会有偏差。
